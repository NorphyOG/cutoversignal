...
import { renderMarkdown, scanRepository, type ScanReport } from "./scanner.ts";

type ActionEnvironment = Record<string, string | undefined>;
...
function required(environment: ActionEnvironment, key: string): string {
...
function resolveScanRoot(workspace: string, candidate: string): string {
...
function resolveReportPath(runnerTemp: string, filename: string): string {
...
function canonicalDirectory(environment: ActionEnvironment, key: string): string {
  try {
...
function reportFilename(environment: ActionEnvironment): string {
...
function writeOutput(path: string, key: string, value: string): void {
  appendFileSync(path, `${key}=${value}\n`, "utf8");
}

export function runAction(environment: ActionEnvironment = process.env): ActionResult {
